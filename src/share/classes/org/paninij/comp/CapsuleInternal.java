...
public class CapsuleInternal extends Internal {
    ...
    protected JCBlock generateThreadCapsuleComputeMethodBody(JCCapsuleDecl tree) {
        ...
        //TODO batch: this is where we add the cases for the batch messages
        //figure out way to make everything pretier
        JCCase batchMessageCase = createBatchMessageLogic();
        cases.append(batchMessageCase);
    ...
    private JCCase createBatchMessageLogic(){
        ListBuffer<JCStatement> buffer = new ListBuffer<JCStatement>();
        ListBuffer<JCExpression> temp = new ListBuffer<JCExpression>();
        JCMethodInvocation duckFutureGetMessage = apply(apply("panini$duck$future", "panini$getMessage"),"apply");
        temp.add(duckFutureGetMessage);
        JCMethodInvocation r = apply("panini$duck$future", "panini$finish", temp);
        buffer.add(make.Exec(r));
        buffer.append(break_());
        return case_(id(PaniniConstants.PANINI_METHOD_CONST_BATCH), buffer);
     }
    ...
    public List<JCClassDecl> generateClassWrappers(JCCapsuleDecl tree,
        ...
        for (List<JCMethodDecl> l = tree.publicMethods; l.nonEmpty(); l = l.tail){
            Name capsuleName = tree.name;
            JCMethodDecl method = l.head;
            Type restype = method.restype.type;
            generateDuckForProcedure(capsuleName, method.name, method.params, restype, classes, alreadedAddedDuckClasses, env);
        }
        return classes.toList();
    }

    public void generateDuckForProcedure(Name capsuleName, Name methodName, List<JCVariableDecl> methodParams, Type restype, ListBuffer<JCClassDecl> classes,
            Map<String, JCClassDecl> alreadedAddedDuckClasses, Env<AttrContext> env) {
        ...
        if(c!=null && !c.toString().equals("java.lang.String")){
            ...
            if (restype.tag == TypeTags.CLASS) {
                if (!alreadedAddedDuckClasses.containsKey(restype.toString())) {
                    JCClassDecl duckClass = generateNewDuckClass(
                            capsuleName.toString(), methodName, methodParams, constructors,
                            restype, iter, c, env);
                    ...
                    if (!methodParams.isEmpty()) {
                        ...
                        adaptDuckClass(methodName, methodParams,restype, iter, duckClass, false);
                    }
                }
                ...
                if (!alreadedAddedDuckClasses.containsKey(restype.toString())) {
                    ...
                    wrappedMethods.add(createBatchGetMessageMethodStub(List.<JCExpression>of(id("Void"))));
                    ...
                    if (!methodParams.isEmpty()) {
                        ...
                        for (List<JCVariableDecl> vl = methodParams; vl
                                .nonEmpty(); vl = vl.tail) {
            ...
            if(restype.isPrimitive()||c.toString().equals("java.lang.String")){
                if (!alreadedAddedDuckClasses.containsKey(restype.toString())) {
                    ...
                    JCClassDecl duckClass = alreadedAddedDuckClasses
                            .get(restype.toString());
                        adaptDuckClass(methodName, methodParams, restype, null, duckClass, true);
                }
            }
        }
    }
    ...
    private JCMethodDecl createDuckConstructor(Iterator<Symbol> iter, boolean isVoid, Name methodName, List<JCVariableDecl> methodParams, JCMethodDecl paniniFinish, final boolean isPrimitive){
        JCMethodDecl constructor = createDuckConstructor(iter, isVoid, isPrimitive);
        for (List<JCVariableDecl> l = methodParams; l.nonEmpty(); l = l.tail){
            ...
            constructor.body.stats = constructor.body.stats.append(es(assign(
                    select(thist(),
                            createFieldString(methodName, par,
                                    methodParams)), id(par.name))));
            if (!par.vartype.type.isPrimitive())
                paniniFinish.body.stats = paniniFinish.body.stats
                        .append(es(assign(
                                select(thist(),
	...
	private void adaptDuckClass(Name methodName, List<JCVariableDecl> methodParams, Type restype, Iterator<Symbol> iter,
        ...
        if (!hasDuplicate(duckClass, methodParams, methodName)) {
            if (paniniFinish == null)
                Assert.error();// ///shouldn't happen
            if (iter == null)
                duckClass.defs = duckClass.defs.append(createDuckConstructor(
                        iter, true, methodName, methodParams, paniniFinish, isPrimitive));
            else
                duckClass.defs = duckClass.defs.append(createDuckConstructor(
                        iter, restype.toString().equals("void"), methodName, methodParams,
                        paniniFinish, isPrimitive));
        }
        for (List<JCVariableDecl> l = methodParams; l.nonEmpty(); l=l.tail){
            JCVariableDecl par = l.head;
            newFields.add(var(mods(PUBLIC), names.fromString(createFieldString(
                    methodName, par, methodParams)), par.vartype));
        }
    ...
    private JCClassDecl generateNewDuckClass(String classNameSuffix,
            Name methodName, List<JCVariableDecl> methodParams, ListBuffer<JCTree> constructors, Type restype,
        ...
        //TODO batch: remember, remember.
        wrappedMethods.add(createBatchGetMessageMethodStub(List.<JCExpression>of(id(c.name.toString()))));
        ...
        if (!methodParams.isEmpty()) {
            for (List<JCVariableDecl> l = methodParams; l.nonEmpty(); l=l.tail){
                ...
                variableFields.add(var(mods(PUBLIC), names
                        .fromString(createFieldString(methodName, par,
                                methodParams)), par.vartype));
            }
            iter = c.members().getElements().iterator();
            constructors.add(createDuckConstructor(iter, false, methodName, methodParams, finishMethod, false));
        }
    ...
    private JCMethodDecl createBatchGetMessageMethodStub(List<JCExpression> genericTypes) {
        ...
        JCMethodDecl value = method(mods(PUBLIC | FINAL), PaniniConstants.PANINI_DUCK_GET_MESSAGE,
                make.TypeApply(id(PaniniConstants.PANINI_BATCH_MESSAGE_TYPE_NAME), genericTypes), params,
                body(returnt(make.Literal(TypeTags.BOT, null))));
        return value;
    }
