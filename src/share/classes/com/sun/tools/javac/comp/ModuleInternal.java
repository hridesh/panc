		...
		for (JCMethodDecl method : tree.publicMethods) {
			...
			if (restype.tag == TypeTags.CLASS) {
				if (!addedHere.containsKey(restype.toString())) {
					JCVariableDecl var = var(mods(PRIVATE), "wrapped", restype.toString(),
							nullv());
					...
					JCClassDecl wrappedClass = make.ClassDef(
							...
							defs(var, var2, varRedeemed, finish, id).appendList(variableFields)
				...
				if (!addedHere.containsKey(restype.toString())) {
					...
					JCClassDecl wrappedClass = make.ClassDef(
							...
							null,
							implement,
							defs(messageId, varRedeemed).appendList(variableFields)
