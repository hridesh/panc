 ...
 *********************************************************************/
    ...
    public void visitConfigDef(JCConfigDecl tree){
    	MethodSymbol msym = new MethodSymbol(
    			...
    			new MethodType(
    					List.<Type>of(syms.stringType),//find a way to get string[]
    					syms.voidType,
    					List.<Type>nil(),
    					syms.methodClass
    					),
    	...
    	memberEnter(maindecl, env);
    	tree.defs = List.<JCTree>of(maindecl);
    	
    	tree.switchToClass();
    }
    
    public void visitModuleDef(JCModuleDecl tree){
    	tree.extending = make.Ident(attr.rs.findType(env, names.fromString("RecursiveAction")));
 
    	ListBuffer<JCTree> definitions = new ListBuffer<JCTree>();
    	boolean haveRun = false;
    	for(int i=0;i<tree.defs.length();i++){
    		if(tree.defs.get(i).getTag() == METHODDEF){
    			...
    			if(mdecl.name.toString().equals("run") && mdecl.params.isEmpty()){
    		    	...
    		    	memberEnter(computedecl, env);
    		    	definitions.add(computedecl);
    		    	haveRun = true;
    			}
    			else{
    				if((mdecl.mods.flags & PROTECTED )==0 
    						&& (mdecl.mods.flags & PRIVATE)==0)
...
//    		make.Assign(make, rhs)
    		definitions.prepend(make.VarDef(make.Modifiers(PRIVATE), 
    				fields.head.name, 
    				fields.head.vartype, 
    				null));
    		fields = fields.tail;
